...
<?
	...
	$saldo = $ouro['ClanMoney'];
	
	//Soma o Saldo existente do tal player, e soma com o que voce desejar depositar.
	$novaouro = $saldo + $valor;
	$sacou = "UPDATE [ClanDb].[dbo].[CL] SET [ClanMoney]='$novaouro' WHERE [ClanName]='$username' ";
	$go = odbc_exec($connection, $sacou);

	if(!$go){
	echo "<div class='notice notice-danger'>
<strong>Erro!</strong> Não foi possivel salvar o saldo.";
	} else {
	echo "<div class='notice notice-sucesso'>
	<strong>Pronto!</strong> Forão entregue o Gold virtual, com sucesso para o lider de sod do clan <strong>$username<strong> So comunicar o lider para pegar o gold na secretaria Karina.";
	} ?>
<?  ?>